...
template<typename Dtype>
void PermuteLayer<Dtype>::Forward_gpu(const std::vector<Blob<Dtype> *> &bottom,
    ...
    if (need_permute_) {
        ...
        PermuteKernel<Dtype> << < CAFFE_GET_BLOCKS(count),
...
template<typename Dtype>
void PermuteLayer<Dtype>::Backward_gpu(const std::vector<Blob<Dtype> *> &top,
